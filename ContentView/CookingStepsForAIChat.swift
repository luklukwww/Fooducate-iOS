...
struct CookingStepsForAIChat: View {
    @Environment(\.dismiss) private var dismiss
    @AppStorage("userUID") private var userUID: String = ""
    
    let ingredients: [AddRecipeView.Ingredient]
    let steps: [String]
    let nutritions: [AddRecipeView.Nutrition]
    let aiResponse: String
    var recipeKnowledgeForKnowledge: Bool = true
    var customStepsForKnowledge: [String]? = nil
    
    // Add initializer for explicit initialization with all parameters
    init(ingredients: [AddRecipeView.Ingredient], 
         steps: [String], 
         nutritions: [AddRecipeView.Nutrition], 
         aiResponse: String, 
         recipeKnowledgeForKnowledge: Bool = true, 
         customStepsForKnowledge: [String]? = nil) {
        self.ingredients = ingredients
        self.steps = steps
        self.nutritions = nutritions
        self.aiResponse = aiResponse
        self.recipeKnowledgeForKnowledge = recipeKnowledgeForKnowledge
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 0) {
                ...
                HStack {
                    ...
                    // Show familiarity status indicator
                    HStack(spacing: 5) {
                ...
                TabView(selection: $currentPage) {
                    // 材料準備頁
                    IngredientsPreparationForAIChat(ingredients: ingredients)
                        .tag(0)
                    
                    // 烹飪步驟頁 - Use displaySteps instead of steps
                    ForEach(Array(displaySteps.enumerated()), id: \.offset) { index, step in
                        StepViewForAIChat(
                            step: step,
                            stepNumber: index + 1,
                            aiTip: aiCookingTip,
                            showAITip: !userUID.isEmpty
                        )
                        .tag(index + 1)
                        .onAppear {
            ...
            .overlay(
                ZStack {
                    ...
                    BackgroundHandGestureDetector(
                        ...
                        onDownSwipe: {
                            if currentPage < (customStepsForKnowledge?.count ?? steps.count) - 1 {
                                currentPage += 1
                            }
                            // Update swipe status
        ...
        .sheet(isPresented: $showTimerSelection) {
            TimerSelectionViewForAIChat(isPresented: $showTimerSelection) { seconds in
        ...
        .onAppear {
            ...
            if let customSteps = customStepsForKnowledge, !recipeKnowledgeForKnowledge {
                displaySteps = customSteps
            } else {
                displaySteps = steps
            }
            
            // Get AI cooking tip for the first step
            if !displaySteps.isEmpty {
                getAICookingTip(step: displaySteps[0])
            }
        ...
        .alert("Cancel Timer?", isPresented: $showCancelTimerConfirm) {
            Button("Cancel Timer", role: .destructive) { cancelTimer() }
            Button("Keep Timer", role: .cancel) { }
        } message: {
            Text("Are you sure you want to cancel the current timer?")
        }
    }
    
    private var totalPages: Int {
        return displaySteps.count + 2 // 材料頁 + 步驟頁 + 完成頁
    }
    ...
    private func speakCurrentContent() {
        ...
        if currentPage == 0 {
            ...
            let ingredientsList = ingredients.map { "\($0.name), \($0.value)" }
        ...
        } else if currentPage < displaySteps.count + 1 {
            // Cooking step stage with better conversational flow
            let stepNumber = currentPage
            let stepContent = displaySteps[currentPage - 1]
    ...
    private func showTimerSetup() {
        // Set initial timer display mode to visible with 0 seconds
        remainingSeconds = 0
        totalSeconds = 0
        timerDisplayMode = .editing
        
        // Show timer setup sheet instead of alert
        showSettings = false
        
        // We'll handle the timer setup directly in the UI
        // The timer display will be shown in the top bar
        // and will be in editing mode
    }
    ...
    // Reset gesture status after delay
...
// 材料準備視圖
struct IngredientsPreparationForAIChat: View {
    let ingredients: [AddRecipeView.Ingredient]
...
// 完成視圖
struct CompletionViewForAIChat: View {
    let onFinish: () -> Void
...
// 添加計時器選擇視圖
struct TimerSelectionViewForAIChat: View {
